const { resize }     = require('./algorithms');
const { ResizeEvent } = require('./events');
...
test('should fit to conditions (randomized tests)', () => {
	for (let i = 0; i < 1000; i++) {
		...
		const { coordinates, restrictions, imageSize, coefficient, aspectRatio, resizeEvent } = conditions;
